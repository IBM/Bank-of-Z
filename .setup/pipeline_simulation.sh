...
export DBB_REPO=${DBB_REPO:-$HOME/sandbox/dbb}
...
export PIPELINE_WORKSPACE=${PIPELINE_WORKSPACE:-$HOME/sandbox/workspace}

# zBuilder framework path (used for DBB_BUILD)
export DBB_BUILD_PATH=${DBB_BUILD_PATH:-$HOME/sandbox/zBuilder}

# Adding pipeline scripts to PATH of the user running this script
export PATH=$PIPELINE_SCRIPTS:$PATH
export TMPHLQ=${TMPHLQ:-fultonm}
...
if [ $rc -eq 0 ]; then
    ...
    dbb build full --hlq FULTONM.BOZ.BLD
    rc=$?
