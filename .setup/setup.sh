...
stage4_build_and_install() {
    ...
    if ! zowe rse-api-for-zowe-cli issue unix-shell \
         "$PIPELINE_WORKSPACE/Bank-of-Z/.setup/build.sh > logs/install_bank_of_z.log 2>&1" \
         --cwd "$PIPELINE_WORKSPACE/Bank-of-Z" &> /dev/null; then
