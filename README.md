...
The setup scripts automatically configure HTTPS on port **9444** using a RACF keyring
and a server certificate signed by the ZVDT image's built-in CA (`VSICA`).

After running setup, one manual step is needed on your local machine to avoid browser
certificate warnings — you must import the image's CA certificate into your OS trust store.

**macOS:**

```bash
# Copy the CA cert from the image (replace 9.114.13.95 with your image IP)
scp ibmuser@9.114.13.95:/u/ibmuser/common_cacert ~/Downloads/boz-ca.pem

# Import and trust it in macOS Keychain
sudo security add-trusted-cert -d -r trustRoot \
  -k /Library/Keychains/System.keychain ~/Downloads/boz-ca.pem
```

Then open **https://\<image-ip\>:9444** in your browser.

> **Note:** Safari requires the server certificate to include an Extended Key Usage
> (EKU) `TLS Web Server Authentication` OID (1.3.6.1.5.5.7.3.1). The certificate
> generated by `addcert.sh` via `RACDCERT GENCERT` does not include EKU because RACF
> does not support setting EKU extensions. Chrome and Firefox work fine; Safari will
> show a "not standards compliant" warning. Contact your z/OS administrator if you
> need a certificate with EKU support (requires an external CA or a custom PKCS#12
> import).
