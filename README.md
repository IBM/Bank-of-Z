...
and the API are served by the same Liberty instance (`BAQ<APP_SHORT_NAME>`), so there is no
...
1. `RACDCERT GENCERT SIGNWITH(CERTAUTH)` — RACF generates the keypair and a CA-signed cert; the private key never leaves RACF.
2. The cert (public side only) is exported as PEM and passed to Bouncy Castle, which rebuilds it with EKU `TLS Web Server Authentication` + SANs, then re-signs it with the VSICA CA key.
...
Re-run `addcert.sh` when the cert expires to generate a fresh one.
