...
Invoke the `OMEN` transaction so that CICS runs `INQCUST` under your user ID.

### Option A — Through the Bank of Z UI

Open the Bank of Z web application and look up a customer whose ID starts with `C` (for example, `C0001`). The UI sends the request through z/OS Connect → CICS → `OMEN` → `INQCUST`.

### Option B — Using a 3270 terminal

Type the transaction ID on an idle CICS screen and press Enter:

```
OMEN
```

Supply the customer number when prompted by the BMS map.
