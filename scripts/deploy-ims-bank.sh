...
substitute_variables() {
    ...
    # Use sed to substitute variables, keep output in ASCII
    # jsub -f will handle the encoding when submitting USS files
    ...
    local sed_rc=$?
    if [[ $sed_rc -ne 0 ]]; then
        log_error "Failed to substitute variables in template: $template"
        return 1
    fi
    
    # Tag the output file as ASCII (ISO8859-1) to match the template
    # jsub -f expects USS files in their native encoding
    chtag -tc ISO8859-1 "$output"
    
    if [[ ! -f "$output" ]]; then
        log_error "Failed to create JCL file: $output"
        return 1
    fi
    
    log_debug "Generated JCL: $output ($(wc -l < "$output") lines)"
    return 0
}
